...
- 支持 Unit ID 扫描（从站发现）：指定一个保持寄存器地址，自动依次探测 Unit ID 1~247，以绿色圆形色块列出有响应的从站，并可随时停止。
...
7. 点击工具栏的 **扫描** 打开 Unit ID 扫描窗口：填写保持寄存器地址后点击 **开始扫描**，依次探测 Unit ID 1~247；有响应的从站显示为绿色色块，按钮前显示已扫描数量，可随时 **停止扫描**。
